...
namespace Assets.Scripts.Views
{
    public class Engines : ViewBasePaging
    {
        ...
        {
            var engines = JsonEngines.Childs.ToList();
            ...
            for (var i = 0; i < engines.Count; i++)
            {
                ...
                var power = GetPower(engines[i]["power"].Value);
                ...
                instance.transform.FindChild("Details").GetComponent<UILabel>().text = string.Format("{0} л.с. ({1})", power, production);
        ...
        {
            var parts = power.Split(Convert.ToChar(" "));
            ...
            return (int) (JsonHelper.GetInt(parts[0]) / 0.73549875);
        }
